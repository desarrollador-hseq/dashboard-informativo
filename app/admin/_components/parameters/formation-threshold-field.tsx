...
export const FormationThresholdField = ({formationParameters}: FormationThresholdFieldProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.patch(`/api/parameters/formation/${formationParameters?.id}`, values);
  ...
  return (
    <div className="mt-6 border bg-slate-200 rounded-md p-4 text-zinc-800">
      <div className="font-medium flex items-center justify-between">
        Porcentaje
        <Button onClick={toggleEdit} variant="default" className="bg-slate-500 hover:bg-slate-700">
          {isEditing ? (
            <><X className="h-5 w-5 " /></>
          ) : (
            <>
        ...
        <Form {...form}>
          ...
          >
            ...
            <div className="flex items-center gap-x-2">
              <Button disabled={!isValid || isSubmitting} className="bg-secondary" type="submit">
                Actualizar
              </Button>
            </div>
          </form>
